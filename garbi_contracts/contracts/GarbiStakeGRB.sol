...
contract GarbiStakeGRB is ReentrancyGuard, Ownable {

    using SafeMath for uint256;
    
    IERC20 public want; // GRB

    uint256 public BLOCK_TO_UNLOCK = 0;
    
    uint256 public PLATFORM_FEE = 0; //0% 0/1000
    ...
    mapping(address => uint256) public withdrawalAvailabilityBlock;
    ...
    constructor(
        ...
        uint256 _blockToUnlock
        ) {
        want = _want;
        miningMachine = _miningMachine;
        pidOfMining = _pidOfMining;
        BLOCK_TO_UNLOCK = _blockToUnlock;
    ...
    function setBlockToUnlock(uint256 _blockToUnlock) public onlyOwner {
        BLOCK_TO_UNLOCK = _blockToUnlock;
    }
    ...
    function deposit(uint256 _wantAmt) external nonReentrant 
    {
        ...
        // Update block when user can withdraw
        withdrawalAvailabilityBlock[msg.sender] = block.number.add(BLOCK_TO_UNLOCK);
    ...
    {
        require(withdrawalAvailabilityBlock[msg.sender] <= block.number, 'INVALID_BLOCK_TO_UNLOCK');
        require(_wantAmt > 0, 'INVALID_INPUT');   
        harvest(msg.sender);
